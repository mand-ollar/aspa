...
class LabelSlaveDataset(Dataset):
    def __init__(
        ...
        assert wav_path is None or audio is None, (
            "Only one of wav_path or audio can be provided."
        )
        ...
        self.audio = format_audio(
            audio=self.audio, sr=audio_sr, new_sr=sr, target_dim=1
        )
        ...
        self.num_windows: int = (
            len(self.audio) - self.window_size
        ) // self.hop_size + 1
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return self.audio[st_pnt:en_pnt].reshape(1, -1), torch.tensor(
            [st_pnt, en_pnt], dtype=torch.int64
        )
