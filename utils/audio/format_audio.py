...
def _format(
    audio: torch.Tensor,
    sr: int,
    new_sr: int | None = None,
    target_dim: int | None = None,
) -> torch.Tensor:
    # Resample if needed
    if new_sr is not None and sr != new_sr:
        audio = torchaudio.functional.resample(
            waveform=audio, orig_freq=sr, new_freq=new_sr
        )
    ...
    if target_dim is not None and audio_dim != target_dim:
        assert audio_size[1] == 1 if audio_dim == 3 else True, (
        ...
        assert audio.dim() == 2, (
            f"Audio must be mono. Got {audio_size} sized audio tensor."
        )

        if target_dim == 1:
            audio = audio.squeeze(dim=0)
            assert audio.dim() == 1, (
                f"Batch size must be 1 for target_dim=1, but got {audio_size[0]}."
            )
        elif target_dim == 2:
            audio = audio
        elif target_dim == 3:
            ...
            raise NotImplementedError(
                f"Target dimension {target_dim} is not supported."
            )
